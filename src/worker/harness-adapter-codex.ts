 ...
 * `harness-nonzero-exit`) — issue #26 AC5 — so the shape can never drift
  ...
  return {
    ...
    async invoke(call: HarnessInvocation): Promise<HarnessResult> {
      ...
      if (spawnResult.idledOut) {
        // Same recovery as the wall-clock branch below (issue #26 AC5): codex
        // streams per-turn usage as it goes, so a run killed for going silent
        // can still carry real, already-billed tokens from turns it completed
        // before it stopped producing output. Distinct code (issue #31) from
        // 'harness-timeout' so the executor and journal can tell a hung call
        // apart from a genuinely slow one, even though both retry the same way.
        fail(
